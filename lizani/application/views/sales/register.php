										...
										<td colspan="5">
											<dl class="register-item-extra-details dl-horizontal">
												...
												if (property_exists($item, 'quantity_units') && count($item->quantity_units) > 0) { ?>
													...
													if ($this->config->item('always_show_item_presentation')) { ?>
														<dt class=""><?php echo lang('common_quantity_units'); ?> </dt>
														<dd class="">

															<a href="#" id="quantity_unit_<?php echo $item->item_number; ?>" data-name="quantity_unit_id" data-type="select" data-pk="1" data-url="<?php echo site_url('sales/edit_item/' . $line); ?>" data-title="<?php echo H(lang('common_quantity_units')); ?>"><?php echo character_limiter(H($item->quantity_unit_id ? $item->quantity_units[$item->quantity_unit_id] : lang('common_none')), 50); ?></a>
														</dd>
														<?php
														$source_data = array();
														...
														?>
														<script>
															$('#quantity_unit_<?php echo $item->item_number; ?>').editable({
															...
															setTimeout(() => {
																if (<?php echo (H($item->quantity_unit_id) ? H($item->quantity_unit_id) : 0); ?> == 0) {
																	$('#quantity_unit_<?php echo $item->item_number; ?>').trigger('click');
																}
												...
												if (property_exists($item, 'prices') && count($item->prices) > 0) { ?>
													...
													foreach ($item->prices as $price) {
														...
														if ($price["value"] !=  "0.0000000000") {
															$source_data[] = array('value' => $price["value"], 'text' => $price["value"]);
															$last = array('value' => $price["value"], 'text' => $price["value"]);
															$index++;
														}
													}
													...
													?>
													<script>
														$('#prices_<?php echo $line; ?>').editable({
															value: <?php echo ($line); ?>,
															source: <?php echo json_encode($source_data); ?>,
															success: function(response, newValue) {
		...
		<?php if (isset($customer)) { ?>
			...
			<div class="customer-badge">
				...
				<div class="details">
					...
					<?php } ?>
					<?php if (isset($customer_credit_limit)) { ?>
						<div class="<?php echo 'text-success'; ?> balance"><?php echo 'Credito' . ': ' .  to_currency($customer_credit_limit); ?></div>
					<?php } ?>
...
<div class="modal fade" id="var_popup" role="dialog" aria-hidden="true">
	<div class="modal-dialog qty-dialog">
		<div class="modal-content">
			...
			<form id="save-qty-form" method="POST" action="<?php echo site_url('sales/add_variations_qty'); ?>">
				<div class="modal-body clearfix">
...
<script type="text/javascript" language="javascript">
	...
	$("#save-qty-form").submit(function(e) {
		...
		if (query.length != 0) {
			...
			$.post('<?php echo site_url('sales/add_variations_qty'); ?>', data, function() {
	...
	$(document).ready(function() {
			...
			autocomplete: {
				...
				create: function() {
					$(this).data('ui-autocomplete')._renderItem = function(ul, item) {
						return $("<li class='item-suggestions rollover'></li>")
				...
				$("#item").autocomplete({
					...
					return $("<li class='item-suggestions'></li>")
						...
						.append('<a  class="suggest-item"><div class="item-image ">' +
							...
							(typeof item.item_number !== 'undefined' && typeof item.item_number !== '' && item.habilita == '1' && item.item_number !== null ? '<span class="attributes">' + 'Codigo: ' + ' <span class="value">' + item.item_number + '</span></span>' : '') +
							(typeof item.manufacturer !== 'undefined' && item.manufacturer !== null ? '<span class="attributes">' + 'Fabricante: ' + ' <span class="value">' + item.manufacturer + '</span></span>' : '') +
				...
				$("#customer").autocomplete({
					...
					return $("<li class='customer-badge suggestions'></li>")
						...
						.append('<a class="suggest-item"><div class="avatar">' +
							'<img src="' + item.avatar + '" alt="" class="img-polaroid" width="45">' +
							'</div>' +
							'<div class="details">' +
							'<div class="name">' +
							item.label +
		...
		$('#show_factura_on_receipt').is(':checked') ? $("#factura_electronica").show() : $("#factura_electronica").hide();
