...
class PHPPOSCartSale extends PHPPOSCart
{
	...
	function add_items(string $nombreproducto, string $cantidad, string $precio, string $total, string $descuento, string $montogravable, string $montoimpuesto, string $linea)
	{

		$item = "<dte:Item NumeroLinea=\"$linea\" BienOServicio=\"S\">" .
			"<dte:Cantidad>" . $cantidad . "</dte:Cantidad>" .
			"<dte:UnidadMedida>UNI</dte:UnidadMedida>" .
			"<dte:Descripcion>" . $nombreproducto . "</dte:Descripcion>" .
			...
			"<dte:MontoGravable>" . str_replace(',', '.', $montogravable) . "</dte:MontoGravable>" .
			"<dte:MontoImpuesto>" . str_replace(',', '.', $montoimpuesto) . "</dte:MontoImpuesto>" .
			"</dte:Impuesto>" .
			"</dte:Impuestos>" .
	...
	function senDataCertificar($xmlFirmado, $nitTramitador, $serie, $number)
	{
		// $xmlEncode = base64_encode($xml);
		$xmlsend = "\r\n{\r\n\t\"dte\": {\r\n\t\"nit_transmitter\": \"$nitTramitador\",\r\n\t\"serie\": \"$serie\", \r\n\t\"number\": $number, \r\n\t\"xml_dte\": \"$xmlFirmado\"\r\n\t}\r\n}";
		...
		curl_setopt_array($curl, array(
			...
			CURLOPT_HTTPHEADER => array(
				"apikey: 046a84cd36e81112a08ba60256cefcd5c5b0da89",
	...
	function senDataSignature($xml, $nitTramitador)
	{
	...
	function anulacion(string $NumeroDocumentoAAnular, string $IDReceptor, string $FechaEmisionDocumentoAnular, string $FechaHoraAnulacion, string $MotivoAnulacion = "Pruebas de anulacion")
	{
        ...
        <dte:SAT>
            <dte:AnulacionDTE ID=\"DatosCertificados\">
                <dte:DatosGenerales ID=\"DatosAnulacion\"
                    NumeroDocumentoAAnular=\"$NumeroDocumentoAAnular\" NITEmisor=\"$NITEmisor\"
	...
	function xmlcontructor(string $nombre, string $nit, string $direccion, string $fecha, string $hora, string $productos, string $total, string $descuento, string $nit_emisor, string $iva)
	{
		$item = $productos;
		$xml = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" .
			...
			"<dte:DatosGenerales Tipo=\"FACT\" FechaHoraEmision=\"" . $fecha . "T" . $hora . "\" CodigoMoneda=\"GTQ\" />" .
			"<dte:Emisor NITEmisor=\"$nit_emisor\" NombreEmisor=\"RICARDO ADOLFO RODRIGUEZ QUEVEDO\" " .
			"CodigoEstablecimiento=\"1\" " .
			"NombreComercial=\"RESTAURANTE MOLINO VIEJO\" AfiliacionIVA=\"GEN\">" .
			"<dte:DireccionEmisor>" .
			"<dte:Direccion>CANTON PACHOJ</dte:Direccion>" .
			"<dte:CodigoPostal>09002</dte:CodigoPostal>" .
			"<dte:Municipio>CHICHICASTENANGO</dte:Municipio>" .
			"<dte:Departamento>QUICHE</dte:Departamento>" .
			"<dte:Pais>GT</dte:Pais>" .
			"</dte:DireccionEmisor>" .
			"</dte:Emisor>" .
			"<dte:Receptor NombreReceptor=\"" . $nombre . "\" CorreoReceptor=\"\" IDReceptor=\"" . $nit . "\">" .
			"<dte:DireccionReceptor>" .
			"<dte:Direccion>" . $direccion . "</dte:Direccion>" .
			"<dte:CodigoPostal>09002</dte:CodigoPostal>" .
			"<dte:Municipio>municipio cliente</dte:Municipio>" .
			"<dte:Departamento>depar cliente</dte:Departamento>" .
			"<dte:Pais>GT</dte:Pais>" .
			"</dte:DireccionReceptor>" .
			"</dte:Receptor>" .
			"<dte:Frases>" .
	...
	*/
	...
	function apply_advanced_discount_mix_and_match($rule, $params)
	{
		...
		foreach ($price_breaks as $price_break_search) {
			if ($total_quantity_in_rule >= (float)$price_break_search['item_qty_to_buy']) {
