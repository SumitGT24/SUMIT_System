...
class Sales extends Secure_area
{
	...
	function item_search()
	{
		...
		if (!$this->config->item('speed_up_search_queries')) {
			...
			for ($k = 0; $k < count($suggestions); $k++) {
				...
				if ($CI->config->item('hide_barcodes_on_search')) {
					$suggestions[$k]['habilita'] = 1;
				} else {
					$suggestions[$k]['habilita'] = 0;
				}
			}
		} else {
		...
		if ($tipo == "NiT") {
			if ($nit == "CF") {
				...
				curl_setopt_array($curl, array(
					CURLOPT_URL => "https://info.totaldoc.io/api/nit/$nit",
					...
					CURLOPT_HTTPHEADER => array(
						"appKey:IB1ce7NU1o;yjfGq9xVEYvfdi0peF-U8NUYx.owHA1GV9zjlp1dCy4JNaHDDLJ,5D.iCY3xVhwfENCtpK5F0ziEFlKdLw58PJp2P1YR5q0US.UA9!k6I7Shd",
						
						
						// app key original "IB1ce7NU1o;yjfGq9xVEYvfdi0peF-U8NUYx.owHA1GV9zjlp1dCy4JNaHDDLJ,5D.iCY3xVhwfENCtpK5F0ziEFlKdLw58PJp2P1YR5q0US.UA9!k6I7Shd",
						// app key para pruebas W30f80c7e8d5c793cf3585a754ae44ef9e0f1ace376a5ccaa936daf6ab3c8537b1bf12160641a9b706998fa31abda0900b4b89236286bbee7d187d4f305cc552c",
			...
			curl_setopt_array($curl, array(
				CURLOPT_URL => "https://info.totaldoc.io/api/cui/$nit",
				CURLOPT_RETURNTRANSFER => true,
				CURLOPT_ENCODING => '',
				CURLOPT_MAXREDIRS => 10,
				CURLOPT_TIMEOUT => 0,
	...
	function add()
	{
		//""4|FORCE_ITEM_ID|"" var general
		// "10#18|FORCE_ITEM_ID|" linda
		// var_dump($this->input->post("item"), $this->input->post("quantity"));
		// exit;			
		$barcode_scan_data = $this->input->post("item");
		$quantity = $this->input->post("quantity");
		...
		try {
			...
			if ($variable == 'quantity_unit_id') {
				...
				if ($qui !== NULL) {
					...
					//prices
					$item->prices[0] = array('value' => $qui->price1, 'price' => $qui->price1);
					$item->prices[1] = array('value' => $qui->price2, 'price' => $qui->price2);
					$item->prices[2] = array('value' => $qui->price3, 'price' => $qui->price3);
	...
	function complete()
	{
		...
		// var_dump($this->cart);
		$prices_unit = [];
		$preCartItems = $this->cart->get_items();
		for ($i = 0; $i < count($preCartItems); $i++) {
			$prices_unit[$i] = $preCartItems[$i]->unit_price;
		}

		// var_dump($preCartItems[0]->unit_price);
		// var_dump($preCartItems);
		// exit();
		$this->load->helper('sale');
		$this->lang->load('deliveries');
		...
		if ($data['store_account_payment']) {
			$data['paid_sales'] = $this->cart->get_paid_store_account_ids();
		}
		...
		if ($this->cart->nit == "") {
			...
			for ($i = 0; $i < count($detalleFEL); $i++) {
				...
				if ($detalleFEL[$i]->allow_alt_description) {
					$descriptionItem = $detalleFEL[$i]->name;
					if ($detalleFEL[$i]->is_serialized) {
						if (!is_null($detalleFEL[$i]->serialnumber) || $detalleFEL[$i]->serialnumber != "") {
							$descriptionItem = $detalleFEL[$i]->name . " SERIE : " . $detalleFEL[$i]->serialnumber . " ";							# code...
						}
					}
				} else {
					if ($detalleFEL[$i]->is_serialized) {
						if (!is_null($detalleFEL[$i]->serialnumber) || $detalleFEL[$i]->serialnumber != "") {
							$descriptionItem = $detalleFEL[$i]->name . " SERIE :" . $detalleFEL[$i]->serialnumber . " ";
						}
					}
				}
				...
				$ivaItem = round(((($totalItem - $descuento) / 1.12) * 0.12), 3);
				$precio = $totalItem;
			...
			$xml = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" .
				...
				"<dte:DatosGenerales Tipo=\"FACT\" FechaHoraEmision=\"" . date('Y-m-d') . "T" . date("H:i:s") . "\" CodigoMoneda=\"GTQ\" />" .
				...
				"NombreComercial=\"$location->company\" AfiliacionIVA=\"GEN\">" .
				"<dte:DireccionEmisor>" .
				"<dte:Direccion>" . $location->address . "</dte:Direccion>" .
				"<dte:CodigoPostal>09002</dte:CodigoPostal>" .
				"<dte:Municipio>" . $location->municipio . "</dte:Municipio>" .
				"<dte:Departamento>" . $location->departamento . "</dte:Departamento>" .
				"<dte:Pais>GT</dte:Pais>" .
				"</dte:DireccionEmisor>" .
				"</dte:Emisor>" .
				"$receptor" .
				"<dte:DireccionReceptor>" .
				"<dte:Direccion>" . $rAddres . "</dte:Direccion>" .
				"<dte:CodigoPostal>09002</dte:CodigoPostal>" .
				"<dte:Municipio>municipio</dte:Municipio>" .
				"<dte:Departamento>cliente</dte:Departamento>" .
				"<dte:Pais>GT</dte:Pais>" .
				"</dte:DireccionReceptor>" .
				"</dte:Receptor>" .
				"<dte:Frases>" .
				...
				"<dte:Adenda>" .
				"<TOTALDOC_TEXTO_EMISOR>Tel/WhatsApp 7760 3205</TOTALDOC_TEXTO_EMISOR>" .		
				"<TOTALDOC_PIE_PAGINA>GRACIAS POR SU COMPRA</TOTALDOC_PIE_PAGINA>".
				"</dte:Adenda>".
			...
			$curl = curl_init();
			curl_setopt_array($curl, array(
				CURLOPT_URL => "https://ingestor.totaldoc.io/v1.0/signature",
				CURLOPT_RETURNTRANSFER => true,
				CURLOPT_ENCODING => '',
				CURLOPT_MAXREDIRS => 10,
				CURLOPT_TIMEOUT => 0,
			...
			$curl = curl_init();
			curl_setopt_array($curl, array(
				CURLOPT_URL => "https://ingestor.totaldoc.io/v1.0/dte",
				CURLOPT_RETURNTRANSFER => true,
				CURLOPT_ENCODING => '',
				CURLOPT_MAXREDIRS => 10,
				CURLOPT_TIMEOUT => 0,
			...
			if ($info->uuid != "") {
				$decodedXML = base64_decode($info->xmlSigned);
				$fileXML = 'facturas/' . $info->uuid . '.xml';
				file_put_contents($fileXML, $decodedXML);
				$sat = 1;

				$serie = $info->serie;
				// $correlativo=$info->numero;
				$this->cart->fell = $info->uuid;
				$data['fell'] =  $info->uuid;
				$this->Sale->update_sale_fell($info->uuid, $sale_id_raw);
				$uuid = $info->uuid;
	...
	function delete($sale_id, $MotivoAnulacion = "sin comentarios")
	{
		$this->check_action_permission('delete_sale');
		...
		$can_delete = TRUE;
		$sale_info = $this->Sale->get_info($sale_id)->row_array();
		// var_dump($this->input->post('anulation'));
		// var_dump($sale_info);
		$sale_info['sale_id'];
		$location = $this->Location->get_info($sale_info['location_id']);
		$NITEmisor = $location->nit;
		$fell = $sale_info['fell'];
		$IDReceptor = $sale_info['nit'];
		$FechaEmisionDocumentoAnular = $sale_info['sale_time'];
		$pieces = explode(" ",  $sale_info['sale_time']);
		...
		<dte:GTAnulacionDocumento xmlns:dte=\"http://www.sat.gob.gt/dte/fel/0.1.0\"
			...
			<dte:SAT>
				<dte:AnulacionDTE ID=\"DatosCertificados\">
					<dte:DatosGenerales ID=\"DatosAnulacion\"
						NumeroDocumentoAAnular=\"$fell\" NITEmisor=\"$NITEmisor\"
						IDReceptor=\"$IDReceptor\" FechaEmisionDocumentoAnular=\"$FechaEmisionDocumentoAnular\"
						FechaHoraAnulacion=\"" . date('Y-m-d') . "T" . date("H:i:s") . "\"
			...
			$curl = curl_init();
			curl_setopt_array($curl, array(
				CURLOPT_URL => "https://ingestor.totaldoc.io/v1.0/signanulacion",
				CURLOPT_RETURNTRANSFER => true,
				CURLOPT_ENCODING => '',
				CURLOPT_MAXREDIRS => 10,
				CURLOPT_TIMEOUT => 0,
			...
			$curl = curl_init();
			curl_setopt_array($curl, array(
				CURLOPT_URL => "https://ingestor.totaldoc.io/v1.0/dte-anulacion",
				CURLOPT_RETURNTRANSFER => true,
				CURLOPT_ENCODING => '',
				CURLOPT_MAXREDIRS => 10,
				CURLOPT_TIMEOUT => 0,
